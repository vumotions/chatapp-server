    ...
    if (err) {
      ...
      if (err instanceof multer.MulterError) {
        ...
        if (err.code === 'LIMIT_FILE_SIZE') {
          return res.status(status.BAD_REQUEST).json(new AppError({ 
        ...
        return res.status(status.BAD_REQUEST).json(new AppError({ 
          message: 'Định dạng file không được hỗ trợ. Chỉ chấp nhận các định dạng: jpeg, jpg, png, gif, mp4, mov, avi, pdf, doc, docx, xls, xlsx, txt, csv.',
          status: status.BAD_REQUEST 
        }))
      }
